...
void AutoPilot<Tcontroller, Tparams>::velocityCommandCallback(
  ...
  if (autopilot_state_ != States::VELOCITY_CONTROL) {
    ...
    ROS_INFO("Switched to VELOCITY_CONTROL state inside callback");
  }

  desired_velocity_command_ = *msg;
  desired_velocity_command_.header.stamp = ros::Time::now();
  std::cout << desired_velocity_command_.twist.linear.z; 
...
AutoPilot<Tcontroller, Tparams>::executeTrajectory(
  ...
  const ros::Time time_now = ros::Time::now();
  // ROS_INFO("************** [%f] Inside execute trajectory*****************",predictive_control_lookahead_);
  if (first_time_in_new_state_) {
    first_time_in_new_state_ = false;
  ...
  ros::Duration offset(0.3);
  reference_state_ = trajectory_queue_.front().getStateAtTime(dt+offset);
  ...
  for (std::list<quadrotor_common::Trajectory>::const_iterator it_trajectories =
    ...
    ROS_INFO("************ [%li] points in traj ***************",it_trajectories->points.size());
    for (std::list<quadrotor_common::TrajectoryPoint>::const_iterator
      ...
      if (it_points->time_from_start.toSec() > (dt.toSec() - time_wrapover)) {
        ...
        // ROS_INFO("************  for time [%f] adding a point at time [%f] ***************", it_points->time_from_start.toSec(),dt.toSec());
        // ROS_INFO("[%f] [%f] point position is [%f], [%f], [%f]", it_points->time_from_start.toSec(),dt.toSec(), it_points->position.x(),it_points->position.y(),it_points->position.z());
        // break;
  ...
  *trajectories_left_in_queue = trajectory_queue_.size();
  // ROS_INFO("**************[%li]Type 2*****************",reference_trajectory_.points.size());
