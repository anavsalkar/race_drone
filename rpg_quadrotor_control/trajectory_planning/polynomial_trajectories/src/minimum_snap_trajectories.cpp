...
PolynomialTrajectory generateMinimumSnapTrajectory(
  ...
  if (num_segments != trajectory_settings.way_points.size() + 1) {
    ROS_ERROR(
        ...
        "(Need num_segments == num_waypoints + 1 for open trajectories.) [%i] [%i]",
        ros::this_node::getName().c_str(), int(num_segments), int(trajectory_settings.way_points.size()));
...
PolynomialTrajectory generateMinimumSnapRingTrajectory(
  ...
  // experimental
  PolynomialTrajectory trajectory = generateMinimumSnapRingTrajectory(
