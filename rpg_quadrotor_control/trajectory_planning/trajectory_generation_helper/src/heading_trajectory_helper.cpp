...
void addHeadingAlongTrajectory(const double initial_heading,
                            const double final_heading,
                            quadrotor_common::Trajectory* trajectory) {
  if (trajectory->points.size() < 2) {
    return;
  ...
  for (it = trajectory->points.begin(); it != trajectory->points.end(); it++) {
    ...
    it->heading = atan2(it->velocity.y(),it->velocity.x())-3.14/2;
    it->heading_rate = (it->velocity.x()*it->acceleration.y()-it->velocity.y()*it->acceleration.x())/(pow(it->velocity.x(),2) + pow(it->velocity.y(),2));
