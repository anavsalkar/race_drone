...
namespace rpg_quadrotor_integration_test {
  ...
  //     << "Did not receive autopilot feedback within 10 seconds.";
    ...
    autopilot_helper_waitForAutopilotFeedback(10.0, kExecLoopRate_);
  ...
  generateMinimumSnapRingTrajectory(segment_times, trajectory_settings,
    ...
    /// For differential-flatness based controller, each point is given as reference
  quadrotor_common::TrajectoryPoint ref_state;
  ros::Time start_sending_traj_cmds = ros::Time::now();
  while (ros::ok()){
    polynomial_trajectories::getPointFromTrajectories(
      trajectory, ros::Duration((ros::Time::now() - start_sending_traj_cmds)));
    autopilot_helper_.sendReferenceState(ref_state);
    if((ros::Time::now() - start_sending_traj_cmds) > segment_times.sum()) break;
    ros::spinOnce();
    command_rate.sleep();
  }
